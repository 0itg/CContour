...
template <index_t n> struct coeff<n, 0>
{
    static int_t const value = 1;
};
template <index_t n> struct coeff<n, n>
{
    static int_t const value = 1;
};
...
template <typename T> constexpr T ipow(T num, unsigned int exp)
{
    if (exp >= sizeof(T) * 8)
        return 0;
    else if (exp == 0)
...
template <typename T> T zeta(T s)
{
    ...
    res *= -1.0 / ((T)ipow(2, ZETA_PRECISION) * ((T)1 - pow(2.0, (T)1 - s)));
    return res;
}